# 👋 Hello, World! I’m Yash Sinha  
**Manager, Digital Services | AI & ML Wizard | Business Intelligence | Project Management **  
📍 New Delhi, India | 🌐 [Portfolio]() | 💼 [LinkedIn](https://www.linkedin.com/in/yashsinha12354)  

---

## 🌟 About Me  
Hey there! I’m a **Technical Program/Product Manager** with **9+ years of experience**, weaving magic with **AI/ML, Generative AI, and Business Intelligence** to solve real-world challenges.  

From **building intelligent chatbots** to **streamlining automotive supply chains**, I transform data into actionable insights while leading projects that deliver real impact.  

💡 My playground? **Google Cloud Platform, Vertex AI, LLMs, RAG frameworks, Power BI, Tableau, Python, and Agile methodologies**.  

I thrive on **pushing boundaries**—whether it’s crafting predictive models, designing interactive dashboards, or mentoring the next generation of innovators. 🚀  

---

## 🛠️ Superpowers  

- ✨ Machine Learning & Data Science  
- 📊 Program & Product Management (PMP Certified)  
- 🤖 Generative AI (RAG, LLMs, GANs, VAEs)  
- 📈 Business Intelligence (Power BI, Tableau, DAX, SQL)  
- 🧠 NLP & Computer Vision  
- 🐍 Python (Pandas, NumPy, TensorFlow, NLTK, SpaCy)  
- ☁️ Cloud Platforms (GCP, AWS, Azure)  
- 🗄️ SAP HR HCM & Data Migration  

---

## 💼 My Adventure Log  

**Program Manager / Product Manager | Data Science & Gen AI**  
*Capital Numbers (Jan 2025 – Present)*  

**Technical Manager | Data Science & Gen AI**  
*Vinove Software & Services (Sep 2024 – Dec 2024)*  

**Technical Manager | Data Science & AI**  
*APCO Holdings, LLC (Jul 2023 – Mar 2024)*  

**Business Analyst / APM | PMO**  
*Safewell (Jan 2022 – Dec 2022)*  

**Operations Manager | PMO**  
*Gautam Engineers Ltd. (Oct 2018 – Jan 2022)*  

**Business Analyst**  
*Capgemini India (Jun 2017 – Jun 2018)*  

---

## 🎓 Knowledge Quest  

- 🎓 **Postgraduate Degree, AI & ML** – Texas McCombs School of Business (2022–2024)  
- 🎓 **M.Tech, Design Engineering** – Centurion University (2020–2022)  
- 🎓 **B.Tech, Mechanical Engineering** – Techno India University (2012–2016)  
- 🎓 **Schooling, Science** – Kendriya Vidyalaya (2003–2012)  
- 📜 **AI & PM Programs** – IIT Roorkee (2024)  

---

## 🏆 Badges of Honor  

- 🥇 **PMP (PMI, 2023)**  
- 📊 **Power BI Data Analyst Associate (Microsoft, 2023)**  
- 📈 **Tableau Certified Consultant (2023)**  
- 🤖 **Salesforce Certified AI Specialist (2023)**  
- 🧠 **NVIDIA: Generative AI, RAG Agents, Brain in 10 Minutes (2025)**  
- 🗄️ **SAP HCM Certified (2018)**  
- ➕ **30+ certifications in AI, ML, BI & PM**  

---

## 🧪 Epic Projects  

- 🤖 **NLP Chatbot for Industrial Safety** (McCombs) – LSTM-based safety risk predictor deployed with Flask/Django  
- 🚗 **Predictive Analytics for Automotive Supply Chain** – Reduced lead times & costs  
- 📊 **Sales Analytics COE** (APCO) – CRM + dashboards for real-time insights  
- 🏥 **EHMS Mobile App** (APCO) – End-to-end development of healthcare solution  
- 🤝 **RPA in Accounting** – Automated 80% processes with RPA bots  

---

## 🤝 Making a Difference  

- 🎓 **Mentor** – AI & ML Programs at BITS Pilani & Texas McCombs  
- ❤️ **Volunteer** – HelpAge India (2004–2008)  

---

## 📣 What’s Cooking?  

- 🛠️ **RAG Developer’s Stack** – Exploring LLMs, vector DBs & GenAI workflows  
- 🎓 **Mentorship** – Coaching pros & grads in AI/ML journeys  
- 🏆 **IIT Roorkee Wins** – Completed AI & PM certifications  

---

## 🚀 Let’s Build Something Epic!  

I’m exploring **Product Management, Project Management, Analytics, and AI/ML** opportunities where I can combine my **technical depth** and **leadership skills**.  

💬 Ping me to geek out on **AI, BI, or Digital Transformation**. Let’s create something game-changing! 🚀  
